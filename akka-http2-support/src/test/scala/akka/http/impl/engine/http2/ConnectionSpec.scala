...
class ConnectionSpec extends AkkaSpecWithMaterializer(
  ...
  "The HTTP/2 connection API" should {
    "establish a secure connection" in {
      ...
      val (queue, response) = Source.queue(10, OverflowStrategy.fail)
      ...
      response.futureValue.head.entity.toStrict(100.millis).futureValue.data shouldBe ByteString("Test successful")
    }
  }
}
